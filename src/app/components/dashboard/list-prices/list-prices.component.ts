...
export class ListPricesComponent implements OnInit {
  ...
  displayedColumns: string[] = ['id', 'label', 'description', 'porcentage', 'actions'];
  dataSource!: MatTableDataSource<any>;
  isDisabled: BooleanInput = false
  colBig!: number;
  colMedium!: number;
  steps!: string[]
  path: string = 'list-prices'
  ...
  constructor(private _routingService: RoutingService, private readonly joyrideService: JoyrideService, private _helpService: HelpService, private _router: Router, private breakpointObserver: BreakpointObserver, private _listPriceService: ListPriceService, private _userService: UserService) {
    this._helpService.helpClient()
    this._routingService.setRouting(`dashboard/${this.path}`, `dashboard/${this.path}`)
...
//   }
  ...
  validateCSD() {
    if (this._userService.user.serial != '' && this._userService.user.postal_code != '') {
      this._router.navigateByUrl(`dashboard/${this.path}/create-list-price`)
    } else {
  ...
  loadData() {
    ...
    this.loading = true
    this._listPriceService.getAllData(this._userService.user.getIdCompany.toString(), this.inactive).subscribe({
      next: (resp) => {
