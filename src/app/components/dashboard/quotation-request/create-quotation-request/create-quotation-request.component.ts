...
@Component({
  selector: 'app-create-quotation-request',
  templateUrl: './create-quotation-request.component.html',
  styleUrls: ['./create-quotation-request.component.scss'],
})
export class CreateQuotationRequestComponent implements OnInit {
  loading = true;
  error: boolean = false;
  error_msg: string = '';
  form: FormGroup;
  ...
  constructor(
    ...
    private _quotationRequestService: QuotationRequestService
  ) {
    ...
    if (this.form.value.id_client) {
      const dialogRef = this.dialog.open(QuotationProductsInventoryComponent, {
        ...
        data: {
          ...
          listPrice: this.typePriceCtrl.value,
        },
      });
  ...
  calculateValors(i: number) {
    if (this.products[i].id_product == 0) {
      ...
      this.products[i].total_percentage =
        (this.typePriceCtrl.value.porcentage *
          this.products[i].price_without_percentage) /
        100;
      this.products[i].price =
        this.products[i].price_without_percentage +
        this.products[i].total_percentage;
    }
    this.products[i].price_unit =
      this.products[i].price / this.products[i].amount_pieces;
    this.products[i].sub_total =
      this.products[i].price * this.products[i].amount;
  ...
  validateProducts() {
    ...
    while (i < this.products.length) {
      if (
        ...
        this.products[i].amount_pieces < 1
      ) {
        isValid = false;
  ...
  create() {
    if (this.validateProducts()) {
      this.loading = true;
      this._quotationRequestService
        .create(this.form.value, this.products)
        .subscribe({
          next: (resp) => {
            Swal.fire({
              title: 'OK',
      ...
      Swal.fire({
        title: 'ERROR',
        text: 'ALGUNOS PRODUCTOS NO ESTAN COMPLETO',
        icon: 'error',
        confirmButtonColor: '#58B1F7',
        heightAuto: false,
      });
