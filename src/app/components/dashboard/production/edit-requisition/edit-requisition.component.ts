...
export class EditRequisitionComponent implements OnInit {
  ...
  constructor(private _route: ActivatedRoute, private _productsOrdersService: ProductOrderService, private _helpService: HelpService, private breakpointObserver: BreakpointObserver, private _uploadService: UploadService, private _formBuider: FormBuilder, private _router: Router, private _userService: UserService, private _listPriceService: ListPriceService, private _orderService: OrderService, private _shippingMethodService: ShippingMethodService) {
    ...
    this.idRequisition = this._route.snapshot.paramMap.get('id') || '0'
  }


  loadData() {
    this._orderService.getData(this.idRequisition.toString()).subscribe({
      next: (resp) => {
        this.order = resp
      },
  ...
  updateProductsOrders(index: number) {
    if (this._productsOrdersService.products_orders[index].amount < 0) {
      this._productsOrdersService.products_orders[index].amount = 0
    }
    else if (this._productsOrdersService.products_orders[index].amount <= this._productsOrdersService.products_orders[index].inventory) {
      ...
      this._productsOrdersService.products_orders[index].amount = Number((this._productsOrdersService.products_orders[index].inventory))
      this._productsOrdersService.products_orders[index].total = Number((this._productsOrdersService.products_orders[index].price * this._productsOrdersService.products_orders[index].amount).toFixed(2))
    }
