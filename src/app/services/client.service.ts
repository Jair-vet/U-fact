...
export class ClientService {
  ...
  createClient(name: string, tradename: string, rfc: string, representative: string, state: string, municipality: string, city: string, postal_code: string, address: string, num_ext: string, num_int: string, telephone: string, email: string, id_company: string, id_regime: string, id_type_price: string, id_seller: string, comments: string, credit_limit: number, credit_days: number, id_residence: number, contacts: Contact[], id_tax: string, country_code: string, address_complete: string) {
    const url = `${base_url}/clients`;
    return this.http.post(url, { name, tradename, rfc, representative, state, municipality, city, postal_code, address, num_ext, num_int, telephone, email, id_company, id_regime, id_type_price, id_seller, comments, credit_limit, credit_days, id_residence, contacts, id_tax, country_code, address_complete }, this.headers).pipe(map((resp: any) => resp.message));
  }
